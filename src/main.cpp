...
// Trapezoid road region: 4 points (bottom-left, bottom-right, top-right, top-left)
// Normalized 0-1: x=0 left, x=1 right; y=0 top, y=1 bottom
static std::vector<cv::Point2f> roadTrapezoid;  // filled on first frame
static bool trapezoidInitialized = false;
static bool pointInRoadTrapezoid(int x, int y, int imgW, int imgH) {
    if (roadTrapezoid.size() != 4) return false;
    cv::Point2f pt((float)x, (float)y);
    return cv::pointPolygonTest(roadTrapezoid, pt, false) >= 0;
}
...
for (int id : keep) {
	...
	// If no match, still draw the TRACK (so you can see ByteTrack output)
	Det* dp = nullptr;
	if (di >= 0) {
	    dp = &dets[di];
	    if (dp->cls != 0) dp = nullptr; // safety
	}
        // bbox centre into angle 
const cv::Rect& angle_box = (dp ? dp->box : tbox);
float cam_angle = bbox_center_angle(angle_box, frame.cols, cam_fov_deg);

        // Check if pedestrian center is in road trapezoid (replaces segNet road mask)
        int cx = tbox.x + tbox.width / 2;
        int cy = tbox.y + tbox.height / 2;
        bool onRoad = pointInRoadTrapezoid(cx, cy, frame.cols, frame.rows);

        // draw bbox: red if on road, green otherwise
        cv::rectangle(frame, tbox, onRoad ? cv::Scalar(0,0,255) : cv::Scalar(0,255,0), 2);
	...
	// Only if we matched a detection this frame: pose + facing + distance + score
	if (dp) {
	    const Det& d = *dp;
	    ...
	    if (d.distance_m > 0.0f)
	        label = cv::format("ID:%zu %.2f %.1fm", tp->getTrackId(), d.score, d.distance_m);
	    else
	        label = cv::format("ID:%zu %.2f", tp->getTrackId(), d.score);

	} else {
	    // Unmatched track: still show it, but no pose/score/dist available
	    label = cv::format("ID:%zu (unmatched)", tp->getTrackId());
	}
	...
	roadTrapezoid = {
	    cv::Point2f(0.25f * w, 0.95f * h),  // bottom-left – narrower
	    cv::Point2f(0.75f * w, 0.95f * h),  // bottom-right
	    cv::Point2f(0.55f * w, 0.45f * h),  // top-right – shifted toward road
	    cv::Point2f(0.45f * w, 0.45f * h)   // top-left
	};
	    trapezoidInitialized = true;
	}
	if (trapezoidInitialized && show) {
	//    std::vector<cv::Point> pts;
	//    for (const auto& p : roadTrapezoid) pts.push_back(cv::Point((int)p.x, (int)p.y));
	//    cv::polylines(frame, std::vector<std::vector<cv::Point>>{pts}, true, cv::Scalar(0, 255, 0), 2);
	}
